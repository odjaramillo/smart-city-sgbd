...
El JOIN con dim_fecha permite filtrar por año/mes/trimestre en Power BI
...
LEFT JOIN dim_clientes_inventario ci
    ON ci.fecha_inicio <= df.fecha
    AND (ci.fecha_fin IS NULL OR ci.fecha_fin > df.fecha)
    AND ci.activo_bool = TRUE
GROUP BY
    df.anio, df.mes, df.nombre_mes, df.trimestre,
    df.dia, df.fecha,
...
JOIN dim_fecha df ON ft.sk_fecha = df.sk_fecha
...
GROUP BY
    GROUPING SETS (
        (df.anio, df.mes, df.dia, df.fecha, dre.subestacion, dre.circuito, dgu.sector_urbano),
        (df.anio, df.mes, df.dia, df.fecha, dre.subestacion, dre.circuito),
        (df.anio, df.mes, df.dia, df.fecha, dre.subestacion),
        (df.anio, df.mes, df.dia, df.fecha)
    )
ORDER BY df.fecha DESC;
...
GROUP BY
    GROUPING SETS (
        (df.anio, df.mes, dre.subestacion, dre.circuito, dre.transformador),
        (df.anio, df.mes, dre.subestacion, dre.circuito),
        (df.anio, df.mes, dre.subestacion),
        (df.anio, df.mes)
    )
ORDER BY df.anio DESC, df.mes DESC;
...
hechos_filtrados AS (
    ...
    JOIN dim_fecha df ON fi.sk_fecha = df.sk_fecha
    LEFT JOIN med_days md ON md.fecha = df.fecha
...
SELECT
    ...
    LPAD(df.anio::TEXT, 4, '0') || '-' || LPAD(df.mes::TEXT, 2, '0') AS periodo,
    -- ==================================================================
...
GROUP BY
    GROUPING SETS (
        ...
        (dre.subestacion, dre.circuito, dre.transformador, df.anio, df.mes, df.nombre_mes, df.trimestre, periodo),
        (dre.subestacion, dre.circuito, df.anio, df.mes, df.nombre_mes, df.trimestre, periodo),
        (dre.subestacion, df.anio, df.mes, df.nombre_mes, df.trimestre, periodo),
        (df.anio, df.mes, df.nombre_mes, df.trimestre, periodo)
        /*
...
ORDER BY df.anio DESC, df.mes DESC, dre.subestacion;
...
JOIN dim_fecha df ON fi.sk_fecha = df.sk_fecha
WHERE fi.excluido_med = FALSE
GROUP BY dt.hora, df.dia_semana, df.dia_semana_num, franja_horaria
ORDER BY df.dia_semana_num, dt.hora;
...
BEGIN
    ...
    FROM dim_fecha df
    JOIN med_fechas mf ON mf.fecha = df.fecha
    WHERE fi.sk_fecha = df.sk_fecha
      AND fi.excluido_med = FALSE;
...
Importar en Power BI en este orden (las dependencias se resuelven solas):
  ...
  Medidas DAX recomendadas (crear en Power BI, no en PostgreSQL):
    - SAIDI YTD:     TOTALYTD([SAIDI], dim_fecha[fecha])
    - SAIFI YTD:     TOTALYTD([SAIFI], dim_fecha[fecha])
